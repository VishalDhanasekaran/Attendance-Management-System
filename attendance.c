...
{
	printf("\n\n\n\n\n\t\t\t\tWelcome everyone!! ATTENDIX is software which is used to maintain student attendance, their informations, and CGPA Calculator.\n");
	return;
...
void displayDevelopers()
{
	gotoxy(70,25);
	SetColor(12);
  	printf("© Developed By VISHAL D");
  	gotoxy(86,26);
  	printf("MOHANAPRIYA V");
  	gotoxy(86,27);
  	printf("PRANAV N T");
  	gotoxy(86,28);
  	printf("AKKSHAT S P M");
  	return;
}
void welcome() 
{
...
int C_smstr_R() 
{
  ...
  scanf("%d", & R1.noOfSubjects);
  for (R1.i = 1; R1.i <= R1.noOfSubjects; R1.i++) 
  {
    fflush(stdin);
...
int admin_home() 
{
  ...
  /*printf("\t\t\t\t\t ==> 6. Save Faculty Details\n");
  printf("\t\t\t\t\t ==> 7. View Faculty Details\n");
  printf("\t\t\t\t\t ==> 8. Search a Faculty by ID\n");
  printf("\t\t\t\t\t ==> 9. Delete Faculty Data\n");
  printf("\t\t\t\t\t ==> 10. Edit Faculty Data\n");*/
  printf("\t\t\t\t\t ==> 11. Log Out\n\n\n");
  SetColor(11);
  printf("\t\t\t\t\t   Select an Option:");
  SetColor(45);
...
int save_student_details() 
{
  ...
  printf("\n   \tEnter Student Address[Street,City,District:] ");
  SetColor(11);
  gets(std.address);
  SetColor(14);
...
int ext() 
{
  ...
  case '1':
    ...
    break;
  case '0':
    printf("\n  THANKS FOR USING ATTENDIX SOFTWARE");
...
int edit2(char id[50], char edit_smstr[20]) {
  ...
  printf("  Enter New Student Branch:");
  gets(std2.section);
...
int student_log_in()
{
	int s=0;
  char name[20], password[20], p;
  char checkstudentID[20] ;
  char checkstudentPassword[20] ;
  ...
  FILE *search;
  search = fopen("Student Info.dll","r");
